...
Melissa.prototype._http = function (options) {
    ...
    const id = (!!options.token ? options.token : (!!this.licenseKey ? this.licenseKey : (!!this.userId ? this.userId : null)));
    if (!!id) {
    ...
    return request(options)
        .then(function (response) {
            if (!isResponseSuccessful(response)) {
                let err = new Error(`${ response.statusCode } - ${ options.url } failed`);
